# 🔗 Useful links

Handy places for reference and extra practice. **Save new links here** — open `content/class7/links/links.md` and add a line like `- [Title](https://…)`.

## Practice questions

- **Best RD Sharma book copy i found online** - [Scribd Link](https://www.scribd.com/document/585778645/RD-Sharma-Question-For-Class-7)


- **RD Sharma Class 7 solutions (BYJU'S)** — free, chapter by chapter. Good for seeing the range of question types; we write our own versions in the same style.
  - [Chapter 1 — Integers](https://byjus.com/rd-sharma-solutions/class-7-maths-chapter-1-integers/)
  - [Chapter 2 — Fractions](https://byjus.com/rd-sharma-solutions/class-7-maths-chapter-2-fractions/)
  - [Chapter 3 — Decimals](https://byjus.com/rd-sharma-solutions/class-7-maths-chapter-3-decimals/)
  - [Chapter 6 — Exponents](https://byjus.com/rd-sharma-solutions/class-7-maths-chapter-6-exponents/)

## Official textbooks

- [NCERT — official textbooks (free PDFs)](https://ncert.nic.in/textbook.php) — the official CBSE maths book and exemplar.

> [!TIP]
> These are for **reference and ideas**. The practice sets on the **Questions** page are our own, written to explain the *why* — not copied from these sites.
